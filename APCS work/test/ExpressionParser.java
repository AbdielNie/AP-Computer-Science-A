...
public class ExpressionParser {
    ...
    public static List<Unit> parse(String expression) throws ExpressionIllegalException {
         ...
         * 若一个Unit�?+，且flag为true，说明该+号表示正�?
         * 若一个Unit�?-，且flag为true，说明该-号表示负�?
         */
        boolean flag = true;
        ...
        for(int i = 0; i < s.length(); i++){
            ...
            char c = s.charAt(i);
            if(c == '∞' || c == 'N' || c == 'p' ||c == 'e' || c >= '0' && c <= '9'){   //操作�?
                flag = false;
                if(s.charAt(i) == '∞'){   //操作数，∞无�?,用于处理上次运算结果为无穷然后继续运算的情形

                    units.add(new Unit(Double.POSITIVE_INFINITY));
                }
                else if(s.charAt(i) == 'N'){   //操作数，NaN，表示未定义的运算结果，�?0/0
                ...
                else if(s.charAt(i) >= '0' && s.charAt(i) <= '9'){   //操作�?

                    double val = s.charAt(i) - '0';
                    boolean hasPoint = false;   //是否出现小数�?
                    double level = 0.1;        //小数部分的单�?
                    int k;
